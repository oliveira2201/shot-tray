...
async function run() {
    try {
        ...
        // Force number to string format usually accepted 55 + DDD + 9 + 8 digits

        console.log(`Sending REAL BUTTONS test (V3) to: ${targetNumber} ...`);
        ...
        const payload = {
            openTicket: "0", // Changed to String
            body: [
                {
                    phone: targetNumber, 
                    title: "Teste Botão V3 (openTicket string) 🚀",
                    body: "Se você estiver vendo este botão, responda OK!",
                    footer: "Rodapé Teste",
                    buttons: [
                        {
                            type: "url",
